...
function buildForms(r: Rules): Record<string, FormDef> {
  ...
  return {
    ...
    kikai: {
      ...
      note: `1台${r.machinePrice}・減価償却${r.depPerMachine}/台`,
    },
    saiyo: {
      fields: [
...
// ルールが変わらない限り作り直さない（記帳行ごとに参照されるため）
let cachedFor: Rules | null = null
let cachedForms: Record<string, FormDef> | null = null

/** いまの数値ルールに対応した記帳フォーム定義 */
export function getForms(): Record<string, FormDef> {
  const r = getRules()
  if (!cachedForms || cachedFor !== r) {
    cachedFor = r
    cachedForms = buildForms(r)
  }
  return cachedForms
}
