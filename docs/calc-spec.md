...
本書は初期プロトタイプ `mock/index.html` の `<script>`（L632–2607）から抽出した**完全な計算・ゲームロジック仕様**であり、現在は **`app/src/lib/calc.ts` がこの仕様の実装**である。`mock/` は 2026-09 に削除済み（削除前の内容は `git show 1ca0437:mock/index.html`）。本文中の行番号は削除前の `mock/index.html` を指す（履歴）。実装が**同一の数値**を再現できるよう、疑似コード・数式・テーブルとして正確に記述する。
